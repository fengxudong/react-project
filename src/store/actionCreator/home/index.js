...
//分类
export function changeClassfiyTop(payload) {
    return {
        type: homeType.UP_CLASSFIFY_TOP,
        payload
    }
}
export function changeClassfiyBottom(payload) {
    return {
        type: homeType.UP_CLASSFIFY_BOTTOM,
        payload
    }
}
...
export default {
    ...
    getSlideList() {
        return async dispatch => {
            ...
            // console.log("数据", data);
            //对分类数据进行二次过滤
            const classify_list_top = data.classify_list.slice(0, 5);
            const classify_list_Bottom = data.classify_list.slice(5, 10);
            //console.log(classify_list_Bottom);

            //更新
            dispatch(changeSlideList(data.slide_list));
            /*演唱会上*/
            dispatch(changeClassfiyTop(classify_list_top));
            /*演唱会下*/
            dispatch(changeClassfiyBottom(classify_list_Bottom));
        }
    },
    //vip
    getDiscountList() {
